...
const Comentario: React.FC<CometnarioProps> = ({ comentario, loged }) => {
  ...
  const removerComentario = async () => {
    await axios.delete(
      `http://localhost:3005/comentario/${comentario.id + ""}`
    );
  ...
  if (comentario.idAutor == loged) {
    BotaoExcluir = () => {
      return (
        <div>
          <button
            className="bg-lime-200 px-6 py-1 mt-2 rounded-md border border-black hover:bg-lime-400 hover:shadow hover:shadow-lime-400"
            ...
            <div className="flex gap-2">
              <a className="flex gap-2" href={theOtherLink}>
                <img
                  ...
                  src={comentario.fotoAlvo}
                  alt=""
                />
              </a>
              <p>Sobre avaliação de {comentario.nomeAlvo}</p>
            </div>
            <p className="w-full bg-white my-2 p-2 rounded-md">
              {comentario.conteudoAlvo}
            </p>
          ...
          <div className="w-full">
            <div className="flex gap-3">
              ...
              <p className="self-center">
                {comentario.nomeAutor.toString()} comentou:
              </p>
            </div>
            <p className="w-full bg-white my-2 p-2 rounded-lg">
              {comentario.conteudo}
            </p>
            <div>
              <BotaoExcluir />
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="bg-lime-500 p-3 rounded-md">
        <div className="flex">
          <Descricao />
        </div>
      </div>
